    ...
    <Section>
      <Section.Header className="flex items-start justify-between gap-5 px-5 py-2">
        <div>
          ...
          <p className="text-sm font-normal text-balance text-foreground/50">Copy and paste this snippet into the header tag of your website</p>
        </div>

        <div>
          <Button onClick={handleClick} className="p-0 h-max" variant="ghost">
            <CopyIcon className="size-4 text-foreground/50" />
          </Button>
        </div>
      </Section.Header>
      <Section.Content className="flex flex-col px-5 py-2">
        <pre>
          <code className="text-foreground/50 whitespace-break-spaces">{snippet}</code>
        </pre>
      </Section.Content>
    </Section>
  )
