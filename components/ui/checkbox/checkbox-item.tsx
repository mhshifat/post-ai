...
export default function CheckboxItem({ title, value, metadata, className, disabled }: CheckboxItemProps) {
  ...
  return (
    <label className={cn("cursor-pointer", className, {
      ...
      <input disabled={disabled} type={type} hidden name={identifier} value={value} checked={isChecked(value) || checkedByDefault} onChange={({ target }) => toggleCheckbox(value, target.checked)} />
